...
# Add in key trial/composite CVD outcomes

## Add CV death data to comorbidities table
...
comorbidities <- comorbidities %>%
  left_join((death_causes %>% select(patid, death_date, cv_death_any_cause, cv_death_primary_cause)), by="patid") %>%
  mutate(predrug_earliest_cv_death_any_cause=ifelse(is.na(cv_death_any_cause) | (!is.na(cv_death_any_cause) & cv_death_any_cause>dstartdate), as.Date(NA), cv_death_any_cause),
         predrug_latest_cv_death_any_cause=predrug_earliest_cv_death_any_cause,
         predrug_cv_death_any_cause=ifelse(!is.na(predrug_earliest_cv_death_any_cause), 1L, 0L),
         postdrug_first_cv_death_any_cause=ifelse(is.na(cv_death_any_cause) | (!is.na(cv_death_any_cause) & cv_death_any_cause<=dstartdate), as.Date(NA), cv_death_any_cause),
         
         predrug_earliest_cv_death_primary_cause=ifelse(is.na(cv_death_primary_cause) | (!is.na(cv_death_primary_cause) & cv_death_primary_cause>dstartdate), as.Date(NA), cv_death_primary_cause),
         predrug_latest_cv_death_primary_cause=predrug_earliest_cv_death_primary_cause,
         predrug_cv_death_primary_cause=ifelse(!is.na(predrug_earliest_cv_death_primary_cause), 1L, 0L),
         postdrug_first_cv_death_primary_cause=ifelse(is.na(cv_death_primary_cause) | (!is.na(cv_death_primary_cause) & cv_death_primary_cause<=dstartdate), as.Date(NA), cv_death_primary_cause)) %>%
  analysis$cached("comorbidities_interim_cv_death", indexes=c("patid", "dstartdate", "drug_substance"))
         

## Define trial/composite comorbidities                                             

trial_mace <- c("primary_incident_mi", "primary_incident_stroke", "cv_death_primary_cause")
trial_hf <- c("primary_hhf")
trial_mace_hf <- c("primary_incident_mi", "primary_incident_stroke", "cv_death_primary_cause", "primary_hhf")
broad_mace <- c("myocardialinfarction", "stroke", "cv_death_any_cause")
broad_hf <- c("heartfailure")

#cvd_comorbs <- list(trial_mace=trial_mace, trial_hf=trial_hf, trial_mace_hf=trial_mace_hf, broad_mace=broad_mace, broad_hf=broad_hf)

cvd_comorbs <- list(trial_mace=trial_mace)


for (i in names(cvd_comorbs)) {
  
    predrug_earliest_varnames <- paste0("predrug_earliest_", cvd_comorbs[[i]])
    predrug_latest_varnames <- paste0("predrug_latest_", cvd_comorbs[[i]])
    predrug_varnames <- paste0("predrug_", cvd_comorbs[[i]])
    postdrug_first_varnames <- paste0("postdrug_first_", cvd_comorbs[[i]])
    
    coalesced_vars <- paste0("COALESCE(", predrug_earliest_varnames, ", '9999-12-31')")
    sql_expr <- paste0("LEAST(", paste(coalesced_vars, collapse=", "), ")")
    new_col <- paste0("predrug_earliest_", i)
    comorbidities <- comorbidities %>%
      mutate(interim_column=sql(sql_expr)) %>%
      mutate(!!new_col:=ifelse(interim_column=='9999-12-31', as.Date(NA), interim_column))

    # coalesced_vars <- paste0("COALESCE(", predrug_latest_varnames, ", '0001-01-01')")
    # sql_expr <- paste0("GREATEST(", paste(coalesced_vars, collapse=", "), ")")
    # new_col <- paste0("predrug_latest_", i)
    # comorbidities <- comorbidities %>% mutate(!!new_col:=sql(sql_expr))
    # 
    # sql_expr <- paste0("GREATEST(", paste(predrug_varnames, collapse=", "), ")")
    # new_col <- paste0("predrug_", i)
    # comorbidities <- comorbidities %>% mutate(!!new_col:=sql(sql_expr))
    # 
    # coalesced_vars <- paste0("COALESCE(", postdrug_first_varnames, ", '9999-12-31')")
    # sql_expr <- paste0("LEAST(", paste(coalesced_vars, collapse=", "), ")")
    # new_col <- paste0("postdrug_first_", i)
    # comorbidities <- comorbidities %>% mutate(!!new_col:=sql(sql_expr))
    
}

comorbidities <- comorbidities %>% analysis$cached("comorbidities_interim_cvd_outcomes", indexes=c("patid", "dstartdate", "drug_substance"))


test <- comorbidities %>% select(patid, dstartdate, drug_substance, predrug_earliest_incident_mi, predrug_earliest_incident_stroke, predrug_earliest_cv_death_primary_cause, predrug_earliest_trial_mace) %>% collect()


#, predrug_latest_incident_mi, predrug_latest_incident_stroke, predrug_latest_cv_death_primary_cause, predrug_latest_trial_mace, predrug_incident_mi, predrug_incident_stroke, predrug_cv_death_primary_cause, predrug_trial_mace, postdrug_first_incident_mi, postdrug_first_incident_stroke, postdrug_first_cv_death_primary_cause, postdrug_first_trial_mace) %>% collect()
