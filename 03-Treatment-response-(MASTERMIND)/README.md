...
Comorbidities included: atrial fibrillation, angina (overall and specifically unstable angina recorded in hospital), anxiety, asthma, benign prostate hyperplasia (BPH), bronchiectasis, CKD stage 5/ESRD, CLD, COPD, cystic fibrosis, dementia, diabetic nephropathy, DKA (hospital data only), falls, family history of diabetes, family history of premature cardiovascular disease, mild/moderate/severe frailty, haematological cancers, heart failure, major and minor amputations in hospital (doesn't only include primary cause), hypertension (uses primary care data only, see note in script), IHD, lower limb fracture, myocardial infarction (overall and more specifically in hospital with a reduced codelists: 'incident_mi'), neuropathy, osteoporosis, other neurological conditions, PAD, photocoagulation therapy (hospital data only), pulmonary fibrosis, pulmonary hypertension, retinopathy, (coronary artery) revascularisation, rheumatoid arthritis, solid cancer, solid organ transplant, stroke (overall and more specifically in hospital with a reduced codelists: 'incident_stroke'), TIA, vitreous haemorrhage (hospital data only), 'primary_hhf' (hospitalisation for HF with HF as primary cause), 'primary_incident_mi' (hospitalisation for MI with MI as primary cause using incident_mi codelist), 'primary_incident_stroke' (hospitalisation for stroke with stroke as primary cause using incident_stroke codelist), osmotic symptoms (micturition control, volume depletion, urinary frequency), genital infection ('medspecific_gi' defined by genital_infection), 'unspecific_gi' (defined by codes from genital_infection_nonspec medcodelist on the same day as code from topical_candidal_meds prodcodelist) and hypoglycaemia.

Non-diabetes medications included: blood pressure medications and diuretics (different classes processed separately: ACE-inhibitors, beta-blockers, calcium channel blockers, thiazide-like diuretics, loop diuretics, potassium-sparing diuretics, ARBs), statins, finerenone, oral steroids, definite genital infection meds (also topical candidal meds used for coding 'unspecific_gi' condition) and oestrogens.
...
| hosp_admission_prev_year | 1 if patient has 1 or more hospital admission in the previous year to drug start (not including dstartdate).<br />NA if no admissions or if HES data not available - changed to 0 if no admissions and HES data available in final merge script | |
...
| MPR_raw_{timewindow}_MFN_hist | History of metformin adherence: NA for all MFN drug periods. For other drugs: MPR_raw_{timewindow} from the first MFN period in patient record (drug_instance == 1) (NA if the metformin period was after dstartdate_class). Medication Possession Ratio (MPR) for current drug class period over specific time window. | timewindow options: 12m (takes date 12-months post initiation or dstopdate_class) and resphba1c (resphba1c takes post_biomarker_12mdate or post_biomarker_6mdate whichever is available in that order); calculated as (total days covered / number of days in time window) * 100 |
| MPR_adj_{timewindow}_MFN_hist | History of metformin adherence: NA for all MFN drug periods. For other drugs: MPR_adj_{timewindow} from the first MFN period in patient record (drug_instance == 1) (NA if the metformin period was after dstartdate_class). Adjusted Medication Possession Ratio (MPR) for current drug class period over specific time window. | timewindow options: 12m (takes date 12-months post initiation or dstopdate_class) and resphba1c (resphba1c takes post_biomarker_12mdate or post_biomarker_6mdate whichever is available in that order); calculated as (total days covered / number of days in time window - days without prescription information) * 100 |
| MPR_strict_{timewindow}_MFN_hist | History of metformin adherence: NA for all MFN drug periods. For other drugs: MPR_strict_{timewindow} from the first MFN period in patient record (drug_instance == 1) (NA if the metformin period was after dstartdate_class). Strict Medication Possession Ratio (MPR) for current drug class period over specific time window. | timewindow options: 12m (takes date 12-months post initiation or dstopdate_class) and resphba1c (resphba1c takes post_biomarker_12mdate or post_biomarker_6mdate whichever is available in that order); calculated as (total days covered / number of days in time window) * 100 but only if the period has at least 3 valid prescriptions (with duration of prescription data) and 0 invalid prescriptions |
| MPR_min1_{timewindow}_MFN_hist | History of metformin adherence: NA for all MFN drug periods. For other drugs: MPR_min1_{timewindow} from the first MFN period in patient record (drug_instance == 1) (NA if the metformin period was after dstartdate_class). Minus 1st prescription Medication Possession Ratio (MPR) for current drug class period over specific time window. | timewindow options: 12m (takes date 12-months post initiation or dstopdate_class) and resphba1c (resphba1c takes post_biomarker_12mdate or post_biomarker_6mdate whichever is available in that order); calculated as (total days covered / number of days in time window) * 100 but removing the information from the first prescription in the period |
...
* [A five-drug class model using routinely available clinical features to optimise prescribing in type 2 diabetes: a prediction model development and validation study](https://www.thelancet.com/journals/lancet/article/PIIS0140-6736(24)02617-5/) Dennis et al. 2025
