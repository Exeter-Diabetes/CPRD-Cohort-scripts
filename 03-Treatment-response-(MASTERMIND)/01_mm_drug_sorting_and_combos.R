...
# Add in insulin start and stop dates (remember, changes in substance not treated as break)

all_scripts <- all_scripts %>%
  group_by(patid) %>%
  dbplyr::window_order(date) %>%
  mutate(INS_startdate=if_else(dstart_substance_Insulin==1, date, as.Date(NA)),
         INS_stopdate=if_else(dstop_substance_Insulin==1, date, as.Date(NA))) %>%
...
## Make table of all insulin prescriptions with type

insulin_from_combinations <- clean_oha_prodcodes %>%
  filter(drug_class_1=="INS" | drug_class_2=="INS") %>%
  mutate(insulin_type = case_when(drug_class_1 == "INS" ~ drug_substance_1,
                                  drug_class_2 == "INS" ~ drug_substance_2,
                                  TRUE ~ NA_character_)) %>%
  select(patid, date, insulin_type) %>%
  analysis$cached("insulin_from_combinations", indexes=c("patid", "date", "insulin_type"))


## Convert to one line per patid/date

all_insulin_scripts <- insulin_from_combinations %>%
  union_all((clean_insulin_prodcodes %>% transmute(patid, date, insulin_type=insulin_cat))) %>%
  analysis$cached("all_insulin_scripts", indexes=c("patid", "date", "insulin_type"))

all_insulin_scripts %>% distinct(insulin_type)


## Add binary variables for insulin type

insulin_scripts_wide_with_window <- all_insulin_scripts %>%
  mutate(basal=as.integer(insulin_type=="Basal insulin"),
         bolus=as.integer(insulin_type=="Bolus insulin"),
         mix=as.integer(insulin_type=="Mix insulin"),
         intermediate=as.integer(insulin_type=="Intermediate insulin")) %>%
  group_by(patid, date) %>%
  summarise(basal=max(basal, na.rm=TRUE),
            bolus=max(bolus, na.rm=TRUE),
            intermediate=max(intermediate, na.rm=TRUE),
            mix=max(mix, na.rm=TRUE),
            .groups="drop") %>%
  mutate(date_plus_6_months = sql("date + INTERVAL 183 DAY")) %>%
  analysis$cached("insulin_scripts_wide_with_window", indexes=c("patid", "date", "date_plus_6_months", "basal", "bolus", "intermediate", "mix"))
  

## Join with main table and find whether within 6 months of each script there was a prescription for each insulin type

insulin_flags <- all_scripts %>%
  select(patid, date) %>%
  left_join(insulin_scripts_wide_with_window, by=join_by(patid==patid, x$date>=y$date, x$date<=y$date_plus_6_months)) %>%
  rename(date=date.x) %>%
  group_by(patid, date) %>%
  summarise(basal_ins_within_6_months=coalesce(as.integer(max(basal, na.rm=TRUE)>0), 0L),
            bolus_ins_within_6_months=coalesce(as.integer(max(bolus, na.rm=TRUE)>0), 0L),
            intermediate_ins_within_6_months=coalesce(as.integer(max(intermediate, na.rm=TRUE)>0), 0L),
            mix_ins_within_6_months=coalesce(as.integer(max(mix, na.rm=TRUE)>0), 0L),
            .groups="drop") %>%
  analysis$cached("insulin_flags", indexes=c("patid", "date"))
...
all_scripts <- all_scripts %>%
  left_join(insulin_flags, by=c("patid", "date")) %>%
...
all_scripts <- all_scripts %>%
  ...
  analysis$cached("all_scripts_interim_5", indexes=c("patid", "date"))
...
all_scripts_a <- all_scripts %>% 
  ...
  ungroup() %>%
  analysis$cached("all_scripts_interim_7a", indexes=c("patid", "date"))
...
all_scripts_b <- all_scripts %>% 
  ...
  mutate(dcnextuse_substance=datediff(lead(date), date),
         dcprevuse_substance=datediff(date, lag(date))) %>%
  ungroup() %>%
  analysis$cached("all_scripts_interim_7b", indexes=c("patid", "date"))
...
all_scripts <- all_scripts_a %>%
  ...
  ungroup() %>%
  analysis$cached("all_scripts_interim_8", indexes=c("patid", "date"))
...
combo_class_start_stop <- combo_class_start_stop %>%
  ...
  select(patid, drug_class_combo, numdrugs_class, dcstartdate, dcstopdate, Acarbose, DPP4, GIPGLP1, Glinide, GLP1, INS, MFN, SGLT2, SU, TZD, INS_startdate, INS_stopdate, basal_ins_within_6_months, bolus_ins_within_6_months, intermediate_ins_within_6_months, mix_ins_within_6_months) %>%
...
combo_substance_start_stop <- combo_substance_start_stop %>%
  ...
  select(patid, drug_substance_combo, numdrugs_substance, dcstartdate, dcstopdate, all_of(drugclasses), all_of(drugsubstances), INS_startdate, INS_stopdate, basal_ins_within_6_months, bolus_ins_within_6_months, intermediate_ins_within_6_months, mix_ins_within_6_months) %>%
...
combo_start_stop <- combo_substance_start_stop %>%
  select(patid, dcstartdate, dcstopdate_substance=dcstopdate, drug_substance_combo, datechange_substance, dateaddrem_substance, dateprevcombo_substance, multi_drug_start_substance, all_of(drugsubstances), timetochange_substance, timetoaddrem_substance, timeprevcombo_substance, INS_startdate, INS_stopdate, basal_ins_within_6_months, bolus_ins_within_6_months, intermediate_ins_within_6_months, mix_ins_within_6_months) %>%
...
combo_start_stop <- combo_start_stop %>%
  ...
  select(patid, dcstartdate, dcstopdate_class, drug_class_combo, timetochange_class, timetoaddrem_class, timeprevcombo_class, multi_drug_start_class, all_of(drugclasses), dcstopdate_substance, drug_substance_combo, timetochange_substance, timetoaddrem_substance, timeprevcombo_substance, multi_drug_start_substance, all_of(drugsubstances), INS_startdate, INS_stopdate, basal_ins_within_6_months, bolus_ins_within_6_months, intermediate_ins_within_6_months, mix_ins_within_6_months) %>%
