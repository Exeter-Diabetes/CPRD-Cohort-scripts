...
drug_substance_start_stop <- all_scripts_long %>%
  ...
  ungroup() %>%
  analysis$cached("drug_substance_start_stop_interim_1", indexes=c("patid", "dstartdate_substance"))
...
drug_substance_start_stop <- drug_substance_start_stop %>%
  ...
  select(patid, drug_class, drug_substance, dstartdate_substance, dstopdate_substance) %>%
...
## Copy down class-specific variables (drug_order, drug_instance, drugline_all) to all drug substance starts within this period
...
drug_start_stop <- drug_substance_start_stop %>%
  ...
  mutate(drug_class_starts=cumsum(!is.na(dstopdate_class))) %>%  #could use any of the variables which are missing for drug substance start only rows
  ungroup() %>%
  ...
  ungroup() %>%
  mutate(substance_start_only=ifelse(is.na(dstopdate_class), 1L, 0L)) %>%
  select(patid, drug_class, dstartdate=dstartdate_substance, dstopdate_class, drug_order, drug_instance, drugline_all, substance_start_only, drug_substance, dstopdate_substance) %>%
  analysis$cached("drug_start_stop", indexes=c("patid", "drug_class", "drug_substance", "dstartdate"))
...
combo_substance_start_stop <- combo_substance_start_stop %>%
  ...
  select(patid, drug_substance_combo, numdrugs_substance, dcstartdate, dcstopdate, all_of(drugsubstances)) %>%
...
combo_start_stop <- combo_substance_start_stop %>%
  ...
  group_by(patid) %>%
  dbplyr::window_order(dcstartdate) %>%
  mutate(drug_class_changes=cumsum(!is.na(dcstopdate_class))) %>%  #could use any of the variables which are missing for drug substance start only rows
  ungroup() %>%
...
combo_start_stop <- combo_start_stop %>%
  ...
  mutate(substance_change_only=ifelse(is.na(dcstopdate_class), 1L, 0L),
         ncurrtx=DPP4+GIPGLP1+GLP1+INS+MFN+SGLT2+SU+TZD-1) %>%
