...
# At the moment: just diabetes drug, biomarker and comorbidity data, and no linkage data
...
all_diabetes_drug_periods <- all_diabetes %>%
  ...
  inner_join((combo_start_stop %>% select(-substance_change_only)), by=c("patid", c("dstartdate"="dcstartdate"))) %>%
