...
# Clean and combine medcodes, ICD10 codes and OPCS4 codes, then merge with index dates
...
## NB: for baseline_biomarkers, cleaning and combining with index dates is 2 separate steps, but as there are fewer cleaning steps for comorbidities I have made this one step here
...
## Clean comorbidity data and combine with index dates
...
for (i in comorbids) {
  ...
  data <- all_codes_clean %>%
    ...
    analysis$cached(index_date_merge_tablename, indexes=c("patid", "index_date"))
...
for (i in comorbids) {
  ...
  comorbidities <- comorbidities %>%
    left_join(pre_index_date, by=c("patid", "index_date")) %>%
    mutate({{pre_index_date_variable}}:=!is.na(pre_index_date_earliest_date_variable)) %>%
    left_join(post_index_date, by=c("patid", "index_date")) %>%
    analysis$cached(interim_comorbidity_table, indexes=c("patid", "index_date"))
}

comorbidities <- comorbidities %>% analysis$cached("comorbidities", indexes=c("patid", "index_date"))
