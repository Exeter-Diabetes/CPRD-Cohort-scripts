...
# This file is for 183 day gap between prescriptions
############################################################################################
...
all_scripts <- all_scripts_interim_6 %>%
  ...
  # create variable needed
  mutate(
    stopgap=ifelse(dcnextuse>183 & drugcombo==lead(drugcombo), 1L, NA),
...
combo_start_stop <- combo_start_stop %>%
  ...
  # create variables needed
  mutate(
    timetochange=ifelse(is.na(lead(dcstartdate)) | datediff(lead(dcstartdate),dcstopdate)>183, datediff(dcstopdate, dcstartdate), datediff(lead(dcstartdate), dcstartdate)),
